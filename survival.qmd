...
# Comparison of CLR and rCLR Transformations Using Microbial Log-Ratio Features in Survival Analysis

To evaluate the impact of compositional preprocessing choices, we compare two common transformations: centered log-ratio (CLR) and robust CLR (rCLR). Using the 15 most prevalent microbial taxa, we compute all pairwise log(x/y) ratios as input features.

Microbiome data are compositional, meaning only the ratios between taxa are meaningful. The log(x/y) transformation expresses the relative abundance of one taxon compared to another on a log scale, making the data more statistically appropriate for modeling and removing compositional constraints.

These features are used in Bayesian Cox models, and the predictive performance of CLR- and rCLR-based models is compared using leave-one-out cross-validation (LOO). This allows us to assess whether robust transformations lead to more reliable or accurate survival predictions in microbiome-based models.
...
After applying moment matching to improve reliability of the leave-one-out estimates, all Pareto $k$ values were below $0.7$, indicating good diagnostic quality.

The rCLR- and CLR-based models showed very similar predictive performance, with no statistically significant difference (e.g., elpd_diff = –0.1, se = 0.1). Results varied slightly between model runs, suggesting that neither transformation consistently outperforms the other in this context.

The model included log-ratio features derived from the 15 most prevalent microbial taxa, resulting in 105 pairwise ratios. Despite the relatively high number of predictors, model fitting remained computationally efficient, completing in a reasonable time without convergence issues.