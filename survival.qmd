...
The rCLR- and CLR-based models showed very similar predictive performance, with no statistically significant difference. The LRA-based model also produced nearly identical results with CLR and rCLR, further confirming that the choice of transformation has little impact on predictive accuracy in this specific analysis. Results varied slightly between model runs, suggesting that none of the transformations consistently outperformed the others.

All models included log-ratio features derived from the 15 most prevalent microbial taxa, resulting in 105 pairwise ratios per dataset. Despite the relatively high number of predictors, model fitting remained computationally efficient and stable across all methods.
