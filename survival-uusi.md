...
# Keep the provided order for those methods that exist; append any others at the end
...
# Plot (color only). Uses a ready palette; remove the scale_* line for ggplot defaults.
ggplot(cindex_metrics, aes(x = model, y = estimate, color = method)) +
  ...
  scale_color_brewer(palette = "Set2", drop = FALSE) +   # <- or comment out for default colors
  labs(
